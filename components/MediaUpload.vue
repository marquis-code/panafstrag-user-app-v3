  ...
  <div class="space-y-4">
    ...
    >
      ...
      <div v-if="!loading" class="relative z-10 flex flex-col items-center">
        ...
        <p class="text-sm font-black text-slate-400 mt-2">{{ t('Click or drag to upload') }}</p>
        <input type="file" @change="handleFileSelect" class="absolute inset-0 opacity-0 cursor-pointer" accept="image/*" />
      </div>

      <div v-if="loading" class="relative z-10 flex flex-col items-center">
        <div class="w-10 h-10 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div>
        <p class="text-sm font-black text-indigo-600 mt-4">{{ t('Uploading...') }}</p>
      </div>
    </div>
    <div v-if="error" class="text-sm font-bold text-red-500">{{ error }}</div>
  </div>
</template>
