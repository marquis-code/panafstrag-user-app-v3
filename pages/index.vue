  ...
  <div class="space-y-16 md:space-y-32 pb-32 bg-white">
    ...
    <section class="container mx-auto px-6">
      <div class="grid md:grid-cols-3 gap-8 md:gap-12">
        <div v-for="(feature, i) in features" :key="i"
          ...
          :class="`delay-${(i + 1) * 100}`">
          <div class="w-14 h-14 bg-black rounded-lg mb-6 md:mb-8 flex items-center justify-center text-white transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3">
    ...
    <section class="container mx-auto px-6">
      <div class="grid lg:grid-cols-2 gap-16 md:gap-24">
        <!-- Objectives -->
        <div class="space-y-8 md:space-y-12 animate-fade-in-up delay-400">
           <div class="border-b border-gray-100 pb-6 md:pb-8">
             <h2 class="text-2xl md:text-3xl font-black tracking-tighter uppercase italic">Institutional <span class="not-italic text-gray-400">Objectives.</span></h2>
           </div>
           <div class="space-y-6">
              <div v-for="(obj, i) in (objectives as any[])" :key="i" class="flex gap-4 md:gap-6 group">
                <span class="text-gray-200 font-black text-3xl md:text-4xl group-hover:text-black transition-colors duration-500">{{ (i+1).toString().padStart(2, '0') }}</span>
                <p class="text-gray-500 font-medium leading-relaxed pt-1 md:pt-2 text-sm md:text-base">{{ obj.description }}</p>
              </div>
        ...
        <!-- Responsibilities -->
        <div class="space-y-8 md:space-y-12 animate-fade-in-up delay-500">
           <div class="border-b border-gray-100 pb-6 md:pb-8">
             <h2 class="text-2xl md:text-3xl font-black tracking-tighter uppercase italic">Core <span class="not-italic text-gray-400">Responsibilities.</span></h2>
           </div>
           <div class="grid gap-4 md:gap-6">
              <div v-for="(res, i) in (responsibilities as any[])" :key="i" class="p-6 md:p-8 bg-gray-50 rounded-2xl hover:bg-black hover:text-white transition-all duration-500 group">
                <p class="font-bold text-xs md:text-sm leading-relaxed">{{ res.description }}</p>
              </div>
    ...
    <section class="container mx-auto px-6">
      <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 md:mb-16 border-b border-gray-100 pb-6 md:pb-8 gap-4">
        ...
        <NuxtLink to="/programs" class="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] hover:text-gray-400 transition-colors self-start md:self-auto">View All Programs —></NuxtLink>
      </div>
      ...
      <div v-else class="grid md:grid-cols-3 gap-8 md:gap-12">
        <div v-for="(program, i) in (programs as any[])" :key="program._id"
          ...
          <div class="aspect-[4/5] bg-gray-100 rounded-2xl overflow-hidden mb-6 md:mb-8 shadow-sm">
            ...
            <div v-else class="w-full h-full flex items-center justify-center text-gray-200 font-black text-5xl md:text-6xl italic">
              PAS
            </div>
          </div>
          <div class="space-y-3 md:space-y-4">
            <span class="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">{{ program.type }}</span>
            <h4 class="text-xl md:text-2xl font-black uppercase tracking-tighter group-hover:text-gray-500 transition-colors line-clamp-2 leading-tight">{{ program.title }}</h4>
            <div class="pt-2 md:pt-4 flex items-center gap-4">
              <NuxtLink :to="`/programs`" class="text-[9px] md:text-[10px] font-black uppercase tracking-[0.1em] border-b-2 border-black pb-1 hover:border-gray-300 transition-all">Read Details</NuxtLink>
            </div>
          </div>
        </div>
    ...
    <section class="container mx-auto px-6">
      <div class="bg-black rounded-[1.5rem] md:rounded-[2rem] p-10 md:p-24 text-center text-white relative overflow-hidden group">
        ...
        <div class="relative z-10 max-w-3xl mx-auto">
          <h2 class="text-3xl md:text-7xl font-black mb-6 md:mb-10 tracking-tighter uppercase italic">Participate in <br class="hidden md:block" /> <span class="not-italic text-gray-400">the Future.</span></h2>
          <p class="text-gray-400 mb-10 md:mb-16 text-base md:text-xl leading-relaxed font-medium">
          ...
          <div class="flex flex-col sm:flex-row items-center justify-center gap-6">
            <button class="btn-secondary !py-3 md:!py-4 !bg-black !text-white !border-white hover:!bg-white hover:!text-black w-full sm:w-auto">
              GET INVOLVED
            </button>
            <NuxtLink to="/contact" class="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] hover:text-gray-400 transition-colors">
              CONTACT OUR GROUP —>
            </NuxtLink>
          </div>
        </div>
